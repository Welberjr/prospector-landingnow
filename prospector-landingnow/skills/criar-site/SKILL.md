...
1. Leia o site atual do lead (web_fetch) e extraia: textos, serviços, contatos, redes sociais e as URLs das imagens (logo, foto do profissional, ambientes, serviços). Se o HTML esconder as imagens (lazy-load), use um subagente para varrer o conteúdo salvo. OBRIGATÓRIO reaproveitar (senão a página fica mais fraca que a atual e o cliente rejeita): (a) FOTOS REAIS, principalmente do escritório/consultório/ambiente e do profissional, baixadas para img/; (b) o FAQ do cliente (perguntas e respostas), para recriar a seção de FAQ; (c) as AVALIAÇÕES do Google/depoimentos reais (com foto do autor quando houver). Nunca inventar depoimento.
...
- RODAPÉ EQUILIBRADO (obrigatório): no DESKTOP as 3 colunas (Navegue, Contato, Onde estamos) ficam SEMPRE lado a lado, dividindo a largura por igual (grid de 3 colunas), nunca com "Onde estamos" caindo para uma linha embaixo. Só no MOBILE vira 2 colunas com "Onde estamos" na linha inteira. Conteúdo centralizado em cada coluna; nunca socado à esquerda com vazio do outro lado.
- CUIDADO com a imagem reaproveitada: print de avaliação do Google, banner com texto, capa de e-book ou card de texto NÃO servem de fundo de faixa (vira texto sobre texto). Conferir o que a imagem é; print de avaliação vira depoimento, não fundo. Fundo de faixa só com foto real e limpa; na dúvida, degradê da faixa 2.0.
- HERO com foto só se for RETRATO do profissional (rosto) ou foto limpa vertical de pessoa. Nunca cortar no círculo do hero foto de ação (levantamento, corrida), foto larga de parede/ambiente ou banner com texto (recorte ruim). Sem foto boa de rosto, fazer o hero CENTRADO só com texto (selo, headline, subtítulo, botões, chips), sem a moldura; as fotos de ambiente/serviço vão para a faixa larga, galeria ou cards.
- FAQ (obrigatório quando o cliente tem): recriar a seção de perguntas frequentes com as perguntas e respostas reais do cliente (reescritas, sem travessões), em acordeão CSS puro ou cards. Fica antes do rodapé.
- FOTOS REAIS DE AMBIENTE (obrigatório quando o cliente tem): usar as fotos reais do consultório/escritório numa faixa ou galeria, e preferir FOTO real (com overlay escuro para leitura) na faixa de destaque em vez do degradê quando a foto for boa e larga. Foto real de ambiente é o que mais convence.
- AVALIAÇÕES REAIS (obrigatório quando o cliente tem): seção de depoimentos com as avaliações reais do Google (nome e, quando houver, foto do autor). Nunca inventar.
